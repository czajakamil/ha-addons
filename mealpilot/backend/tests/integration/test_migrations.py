...
def _legacy_db(tmp_path):
    ...
    with eng.begin() as conn:
        ...
        conn.execute(text(
            "INSERT INTO users (username, password_hash) VALUES ('stary', 'hash')"
        ))
        conn.execute(text(
            "INSERT INTO recipes (id, user_id, title, servings) VALUES ('r1', 1, 'Stary przepis', 2)"
        ))
    return eng
...
def test_migrate_adds_new_columns_and_keeps_data(tmp_path):
    ...
    assert {"can_use_ai", "ai_monthly_token_limit", "ai_used_tokens_this_month",
            "ai_usage_period_start"} <= user_cols

    recipe_cols = {c["name"] for c in insp.get_columns("recipes")}
    assert {"image_filename", "owner_user_id", "owner_household_id",
            "meal_types", "is_meal_prep", "meal_prep_days", "meal_prep_steps"} <= recipe_cols
